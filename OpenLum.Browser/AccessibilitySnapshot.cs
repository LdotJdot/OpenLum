...
internal static class AccessibilitySnapshot
{
    ...
    public static async Task<(string Snapshot, Dictionary<
            string,
            (string Role, string? Name, int Nth)
        > RefMap)> GetSnapshotAsync(
        IPage page,
        int maxChars = 15000,
        int maxNodes = 500,
    ...
    private static List<AxNode> ParseAxNodes(JsonElement? response)
    {
        ...
        if (response is null)
            return list;
        if (
            !response.Value.TryGetProperty("nodes", out var nodesEl)
    ...
    {
        if (!node.TryGetProperty(prop, out var el))
            return null;
    ...
    private static (string Snapshot, Dictionary<
        string,
        (string Role, string? Name, int Nth)
    > RefMap) FormatSnapshot(List<AxNode> nodes, int maxNodes, int maxChars)
    {
        ...
        // 经典模式：仅输出带 ref 的交互元素（button/link/textbox 等），便于通过 ref 直接点击/输入
        foreach (var n in nodes)
        {
            if (nodeCount >= maxNodes || charCount >= maxChars)
                break;
            if (n.Ignored)
                continue;
            ...
            refCounter++;
            var refId = refCounter.ToString();
            refMap[refId] = (role, string.IsNullOrEmpty(name) ? null : name, nth);

            var line =
                $"[{refId}] {role}"
                + (string.IsNullOrEmpty(name) ? "" : $" \"{Escape(name)}\"")
