...
public sealed class ExecTool : ITool
{
    ...
    public async Task<string> ExecuteAsync(IReadOnlyDictionary<string, object?> args, CancellationToken ct = default)
    {
        ...
        // 校验随包 exe 是否存在，避免 LLM 幻觉调用不存在的文件（Skills/ 或 InternalTools/；webbrowser 已改为 shell 调用 agent-browser）
    ...
    private bool TryExtractSkillExePath(string command, out string fullPath)
    {
        ...
        if (string.IsNullOrEmpty(candidate) || !candidate.EndsWith(".exe", StringComparison.OrdinalIgnoreCase))
            return false;
        if (!LooksLikeBundledRelativeToolPath(candidate))
            return false;
        ...
        foreach (var baseDir in bases)
        {
            ...
            if (LooksLikeBundledRelativeToolPath(resolved))
            {
                fullPath = resolved;
                return true;
            }
    ...
    private bool TryResolveSkillExeWorkingDir(string command, out string exeDir)
    {
        ...
        foreach (var baseDir in bases)
        {
            ...
            if (File.Exists(fullPath) && LooksLikeBundledRelativeToolPath(fullPath))
            {
    ...
    /// <summary>Path under app or workspace that points at Skills/ or InternalTools/ bundled content.</summary>
    private static bool LooksLikeBundledRelativeToolPath(string path)
    {
        if (string.IsNullOrEmpty(path)) return false;
        return path.Contains("skills", StringComparison.OrdinalIgnoreCase)
            || path.Contains("InternalTools", StringComparison.OrdinalIgnoreCase);
    }
}
