...
- exe 路径（如 `skills/webbrowser/browser/openlum-browser.exe`）
- 命令格式、子命令、参数
- 示例命令
- 错误处理与注意事项
...
**示例：webbrowser skill 的典型调用链**

| 步骤 | 模型动作 | 结果 |
|------|----------|------|
| 1 | 根据 description 选 webbrowser | - |
| 2 | read(skills/webbrowser/SKILL.md) | 获得 exe 路径、navigate/type/click 等命令格式 |
| 3 | exec: openlum-browser.exe --visible navigate --url "https://cn.bing.com" | 浏览器打开，返回 snapshot |
| 4 | 从 snapshot 找到搜索框 ref | - |
| 5 | exec: openlum-browser.exe type --ref 2 --text "关键词" --submit | 输入并搜索 |
