...
```
Application.Run()
    └── 解析 hostRoot（openlum.json hostRoot 或 OPENLUM_HOST_ROOT，默认 exe 目录）
    └── RuntimeHostPaths.HostRoot = hostRoot
    └── SystemPromptBuilder.Build(workspaceDir, hostRoot, tools)
            └── BuildSkillsSection(hostRoot)
                    ├── SkillLoader.Load(hostRoot)       // 仅扫描宿主目录下的 skills/
                    └── SkillLoader.FormatForPrompt(...)
```

**1. 目录扫描（SkillLoader.Load）**

- **不再**从用户 `workspace` 下扫描 skill；用户工程目录只作为 **Workspace**，与宿主资源分离。
- 在 **host root**（宿主根目录）下查找 `skills/` 或 `Skills/`，其下每个子目录若含 `SKILL.md` 即为一项 skill。宿主根目录还应包含与 `skills` **同级**的 `InternalTools/`（内置提取器等）。
- `hostRoot` 默认：`AppContext.BaseDirectory`（与 `openlum-console` 同目录）。可通过配置 `hostRoot` 或环境变量 `OPENLUM_HOST_ROOT` 指向统一的宿主文件夹。

逻辑要点：在 `hostRoot/skills`（或 `Skills`）下枚举子目录，存在 `SKILL.md` 则加入列表；同名以首次为准。详见仓库内 `SkillLoader.cs`。
...
- 执行方式（如 webbrowser 通过 shell 调用 `agent-browser`；内置 `read` 对 PDF/Office/CAD 使用 `InternalTools/read/.../xxx.exe`，与 Skills 元数据分离）
- 命令格式、子命令、参数
- 示例命令
- 错误处理与注意事项
