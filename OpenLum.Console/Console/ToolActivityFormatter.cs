...
public static class ToolActivityFormatter
{
    public static string FormatStarting(string toolName, string? argsJson)
    {
        return toolName.Trim().ToLowerInvariant() switch
        {
            ...
            "semantic_search" => "Searching workspace…",
    ...
    public static string FormatCompleted(string toolName, string? argsJson, bool success)
    {
        ...
        try
        {
            ...
            "semantic_search" => FormatSemanticSearchCompleted(root),
    ...
    private static string FormatReadCompleted(JsonElement root)
    {
        ...
        var offset = GetInt(root, "offset", 1);
        var limit = GetInt(root, "limit", 200);
        if (offset == 1 && limit >= 2000)
            return $"Read {label}";
        var end = offset + Math.Max(0, limit - 1);
        return $"Read {label} L{offset}-{end}";
    }
    ...
    private static string FormatSemanticSearchCompleted(JsonElement root)
    {
        var q = Truncate(GetString(root, "query"), 56);
        if (string.IsNullOrEmpty(q)) return "Searched workspace";
        var path = GetString(root, "path");
        if (string.IsNullOrWhiteSpace(path)) path = ".";
        return $"Searched workspace for {q} in {DescribePathTarget(path)}";
    }
