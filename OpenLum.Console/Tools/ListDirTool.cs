...
public sealed class ListDirTool : ITool
{
    ...
    public Task<string> ExecuteAsync(IReadOnlyDictionary<string, object?> args, CancellationToken ct = default)
    {
        ...
        if (fullPathsShallow)
            return Task.FromResult(ListSingleLevelPaths(fullPath, maxLines));

        return Task.FromResult(ListSingleLevelTable(fullPath));
    }

    private static string ListSingleLevelTable(string fullPath)
    {
    ...
    private static string ListSingleLevelPaths(string fullPath, int maxLines)
    {
        List<string> lines = [];
        ...
        {
            if (IsHiddenName(Path.GetFileName(p))) continue;
